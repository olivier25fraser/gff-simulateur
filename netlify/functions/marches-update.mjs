...
export default async function handler() {
  ...
  const indices = [
    ...
    { key: "tsx",    syms: ["EWC","XIC.TO","XIU.TO"], nom: "TSX Composite", pays: "Canada" },
  ];
  ...
  // Pétrole WTI — essayer plusieurs symboles (USOIL, CL1!, BZ1!, USO)
  const oilSymbols = ["USOIL", "CL1!", "USO"];
  let oilDone = false;
  for (const sym of oilSymbols) {
    if (oilDone) break;
    try {
      const oil = await td("time_series", { symbol: sym, interval: "1day", outputsize: "5", order: "DESC" });
      if (oil.status === "error") { diag.push(`oil/${sym}: ${oil.message}`); continue; }
      const vals = oil.values || [];
      // Prendre les 2 premières valeurs non-nulles
      const valid = vals.filter(v => v.close && parseFloat(v.close) > 0);
      if (valid.length >= 2) {
        const now  = parseFloat(valid[0].close);
        const prev = parseFloat(valid[1].close);
        const chg  = prev ? ((now / prev - 1) * 100) : 0;
        // USO est un ETF ~1/10 du WTI — ajuster si nécessaire
        const factor = sym === "USO" ? 10 : 1;
        data.commodities.oil = { price: (now * factor).toFixed(2), changePct: chg.toFixed(2) };
        diag.push(`oil/${sym}: ${(now*factor).toFixed(2)} (${chg.toFixed(2)}%)`);
        oilDone = true;
      } else { diag.push(`oil/${sym}: série vide`); }
    } catch (e) { diag.push(`oil/${sym}: ${e.message}`); }
  }
  if (!oilDone) diag.push("oil: tous les symboles ont échoué");
