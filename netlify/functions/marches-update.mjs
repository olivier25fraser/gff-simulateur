...
export default async function handler() {
  ...
  // Pétrole WTI (USOIL sur Twelve Data)
  try {
    const oil = await td("time_series", { symbol: "USOIL", interval: "1day", outputsize: "2", order: "DESC" });
    if (oil.values && oil.values.length >= 2) {
      const now  = parseFloat(oil.values[0].close);
      const prev = parseFloat(oil.values[1].close);
      const chg  = prev ? ((now / prev - 1) * 100) : 0;
      data.commodities.oil = { price: now.toFixed(2), changePct: chg.toFixed(2) };
      diag.push(`oil: ${now} (${chg.toFixed(2)}%)`);
    } else { diag.push("oil: série vide"); }
  } catch (e) { diag.push("oil: " + e.message); }
