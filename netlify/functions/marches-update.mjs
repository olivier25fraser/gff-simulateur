...
export default async function handler() {
  ...
  // facteur = ratio ETF → valeur réelle de l'indice (l'ETF réplique l'indice à l'échelle)
  // Les rendements (%) ne sont PAS affectés par le facteur (ce sont des ratios)
  const indices = [
  ...
  for (const idx of indices) {
    ...
    for (const sym of idx.syms) {
      try {
        ...
        // Le prix affiché = prix ETF × facteur (pour ressembler à l'indice réel)
        // Les rendements (returns) restent inchangés car ce sont des pourcentages
        data.indices[idx.key] = {
          nom: idx.nom, pays: idx.pays,
          price: (now * fact).toFixed(2), changePct: changePct.toFixed(2),
          ...returns,
        };
        delete data.indices[idx.key].prix;
