...
IMPORTANT: Ta réponse doit commencer DIRECTEMENT par le caractère [ et finir par ]. Aucune phrase d'introduction comme \"Voici\", aucun texte avant ou après, aucun backtick. UNIQUEMENT le tableau JSON brut. Format:
[
  {
...
Génère exactement 6 nouvelles. Priorise les événements géopolitiques, décisions des banques centrales, données économiques importantes, et crises qui font bouger les marchés. Toujours en français québécois.`,
    ...
    // Parser le JSON de façon robuste (Claude ajoute parfois du texte autour)
    let clean = textBlocks.replace(/```json|```/g, '').trim();
    // Extraire le tableau JSON même s'il y a du texte avant/après (ex: "Voici les nouvelles: [...]")
